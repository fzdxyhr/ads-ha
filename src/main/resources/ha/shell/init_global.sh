...
sed -i "s/^localhost=.*$/$localhost/g" $PATH/global.sh
interfaceName="interface_name=$interface_name"
sed -i "s/^interface_name=.*$/$interfaceName/g" $PATH/global.sh
gatewayIp="gatewayIp=$gateway_ip"
sed -i "s/^gatewayIp=.*$/$gatewayIp/g" $PATH/global.sh
otherIp="otherIp=$otherIp"
sed -i "s/^otherIp=.*$/$otherIp/g" $PATH/global.sh
vrrpIp="vrrpIp=$1"
sed -i "s/^vrrpIp=.*$/$vrrpIp/g" $PATH/global.sh
